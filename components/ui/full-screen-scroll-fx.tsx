...
export type FullScreenFXProps = {
  ...
  /** When true, skip ScrollTrigger/pinning — driven externally via currentIndex */
  embedded?: boolean;
};
...
export const FullScreenScrollFX = forwardRef<HTMLDivElement, FullScreenFXProps>(
  (
    ...
    useLayoutEffect(() => {
      ...
      if (!fixed || ((!fs || total === 0) && !embedded)) return;
    ...
    }, [total, initialIndex, motionOff, bgTransition, parallaxAmount, embedded]);

    // In embedded mode, respond to external currentIndex changes
    const prevEmbeddedIndex = useRef(index);
    useEffect(() => {
      if (!embedded) return;
      if (prevEmbeddedIndex.current !== index) {
        changeSection(index);
        prevEmbeddedIndex.current = index;
      }
    }, [embedded, index]);
    ...
    return (
      <div
        ...
        style={{ ...cssVars, ...style, ...(embedded ? { height: '100%' } : {}) }}
        aria-label={ariaLabel}
      >
        {debug && <div className="fx-debug">Section: {index}</div>}

        {embedded ? (
          <div className="fx-fixed" ref={fixedRef} style={{ position: 'relative', height: '100%', width: '100%', overflow: 'hidden', background: 'var(--fx-stage-bg)' }}>
            {/* Backgrounds */}
